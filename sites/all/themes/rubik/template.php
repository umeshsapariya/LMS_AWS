...
function rubik_preprocess_page(&$vars) {
	global $user;
  // Call center Head
  if (isset($user->roles[14])) {
    //variable_set("call_head", $vars['primary_local_tasks']);
    $call_head = variable_get("call_head", array());
    unset($call_head[0]);
		unset($call_head[2]);
		unset($call_head[5]);
    unset($call_head[7]);
    ...
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($call_head as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $call_head[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $call_head[$key]['#active'] = TRUE;
      }
      elseif (isset($call_head[$key]['#active'])) {
        $call_head[$key]['#active'] = FALSE;
      }
    }
    $vars['primary_local_tasks'] =  $call_head;
  }
  // Supervisor User.
  if (isset($user->roles[11])) {
    /*dsm($vars['primary_local_tasks']);
		variable_set("supervisor_tab", $vars['primary_local_tasks']);*/
    $supervisor_tab = variable_get("supervisor_tab", array());
    unset($supervisor_tab[0]);
		unset($supervisor_tab[2]);
		unset($supervisor_tab[5]);
    unset($supervisor_tab[7]);

    // set active tab.
    $request_uri = request_uri();
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($supervisor_tab as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $supervisor_tab[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $supervisor_tab[$key]['#active'] = TRUE;
      }
      elseif (isset($supervisor_tab[$key]['#active'])) {
        $supervisor_tab[$key]['#active'] = FALSE;
      }
    }
    $vars['primary_local_tasks'] =  $supervisor_tab;
  }

  // Call Center Agent.
	if (isset($user->roles[4])) {
    $agent_tab = variable_get("agent_tab", array());
    unset($agent_tab[0]);
		unset($agent_tab[2]);
		unset($agent_tab[5]);
    ...
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($agent_tab as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $agent_tab[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $agent_tab[$key]['#active'] = TRUE;
      }
      elseif (isset($agent_tab[$key]['#active'])) {
        $agent_tab[$key]['#active'] = FALSE;
      }
    }
	  $vars['primary_local_tasks'] =  $agent_tab;
  }
  ...
  // Data Entry Operator.
    ...
    $data_entry_tab = variable_get("data_entry_tab", array());
		unset($data_entry_tab[0]);
		unset($data_entry_tab[2]);
		unset($data_entry_tab[3]);

    // set active tab.
    $request_uri = request_uri();
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($data_entry_tab as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $data_entry_tab[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $data_entry_tab[$key]['#active'] = TRUE;
      }
      elseif (isset($data_entry_tab[$key]['#active'])) {
        $data_entry_tab[$key]['#active'] = FALSE;
      }
    }

    $vars['primary_local_tasks'] =  $data_entry_tab;
  }

  // BBD center User.
  if (isset($user->roles[6])) {
    $bbd_center_tab = variable_get("bbd_center_tab", array());
    unset($bbd_center_tab[0]);
    unset($bbd_center_tab[2]);
    unset($bbd_center_tab[3]);
  
    // set active tab.
    $request_uri = request_uri();
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($bbd_center_tab as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $bbd_center_tab[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $bbd_center_tab[$key]['#active'] = TRUE;
      }
      elseif (isset($bbd_center_tab[$key]['#active'])) {
        $bbd_center_tab[$key]['#active'] = FALSE;
      }
    }
    $vars['primary_local_tasks'] =  $bbd_center_tab;
  }
  
  // Finance team.
  if (isset($user->roles[8])) {
    $finance_tab = variable_get("finance_tab", array());
    unset($finance_tab[0]);
    unset($finance_tab[2]);
    unset($finance_tab[3]);
    ...
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($finance_tab as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $finance_tab[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $finance_tab[$key]['#active'] = TRUE;
      }
      elseif (isset($finance_tab[$key]['#active'])) {
        $finance_tab[$key]['#active'] = FALSE;
      }
    }
    $vars['primary_local_tasks'] =  $finance_tab;
  }
  
  // Approval Committee.
  if (isset($user->roles[7])) {
    $approval_tab = variable_get("approval_tab", array());
    unset($approval_tab[0]);
    unset($approval_tab[1]);
    unset($approval_tab[3]);
  
    // set active tab.
    $request_uri = request_uri();
    // Remove the / form from url.
    $request_uri = substr($request_uri, 1);
    foreach ($approval_tab as $key => $tab) {
      if (isset($tab['#link']['href'])) {
         $link_path = str_replace('%', $user->uid ,$tab['#link']['path']);
         $approval_tab[$key]['#link']['href'] = $link_path;
      }
      if (isset($tab['#link']['href']) && $link_path == $request_uri) {
        $approval_tab[$key]['#active'] = TRUE;
      }
      elseif (isset($approval_tab[$key]['#active'])) {
        $approval_tab[$key]['#active'] = FALSE;
      }
    }
    $vars['primary_local_tasks'] =  $approval_tab;
  }
  $current_path = explode('/',$_SERVER['REQUEST_URI']);
	//set redirection based on user role
	if((drupal_is_front_page() && isset($user->roles[13])) || (isset($user->roles[13]) && $current_path[1] == 'users')) {
		drupal_goto('user/'.$user->uid.'/data-entry-operator');
	}	
	if((drupal_is_front_page() && isset($user->roles[11])) || (isset($user->roles[11]) && $current_path[1] == 'users')) {
		drupal_goto('user/'.$user->uid.'/manage-enquiries-supervisor');
	}	
	if((drupal_is_front_page() && isset($user->roles[4])) || (isset($user->roles[4]) && $current_path[1] == 'users')) {
		drupal_goto('user/'.$user->uid.'/manage-enquiries-agent');
	}
  // BBD center redirect
  if((drupal_is_front_page() && isset($user->roles[6])) || (isset($user->roles[6]) && $current_path[1] == 'users')) {
		drupal_goto('user/'.$user->uid.'/bbd-centre/search');
	}
  // Approval committee
  if((drupal_is_front_page() && isset($user->roles[7])) || (isset($user->roles[7]) && $current_path[1] == 'users')) {
		drupal_goto('user/'.$user->uid.'/evaluate-application'); 
	}
  // Finance team
  if((drupal_is_front_page() && isset($user->roles[8])) || (isset($user->roles[8]) && $current_path[1] == 'users')) {
		drupal_goto('user/'.$user->uid.'/create-franchisee-by-finance-team'); 
	}
